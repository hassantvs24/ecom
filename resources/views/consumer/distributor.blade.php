...
@section('content')
    <div class="row">
        <div class="col-md-6">
            <p><button type="button" class="btn btn-primary btn-labeled reg" data-level="Distributor" data-toggle="modal" data-target="#regModal"><b><i class="icon-file-plus"></i></b> Add New Distributor</button></p>
        </div>
        ...
        <div class="col-md-12">
            <div class="panel panel-flat border-top-success">
                <div class="panel-heading">
                    <h6 class="panel-title">Distributor List</h6>
                </div>
                        ...
                        @foreach($table as $row)
                            ...
                            <!--<td>{{money_c(round($row->points))}}</td>-->
                                <td class="text-right white_sp">
                                    <button class="btn btn-xs btn-success no-padding mr-5 edipBtn"  data-location="{{$row->locationID}}" data-country="{{$row->country}}" data-name="{{$row->name}}" data-contact="{{$row->contact}}" data-post="{{$row->postCode}}" data-city="{{$row->city}}" data-state="{{$row->state}}" data-address="{{$row->address}}" data-id="{{$row->id}}"  data-toggle="modal" data-target="#edipModal" title="Edit Profile Information"><i class="icon-pencil5"></i></button>
